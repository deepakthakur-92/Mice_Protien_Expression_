...
class Raw_Data_validation:
    ...
    def createDirectoryForGoodBadRawData(self):
        ...
        try:
            path = os.path.join("Training_Raw_Data_files_validated/", "Good_Raw/")
            if not os.path.isdir(path):
                os.makedirs(path)
            path = os.path.join("Training_Raw_Data_files_validated/", "Bad_Raw/")
            if not os.path.isdir(path):
                os.makedirs(path)
    ...
    def moveBadFilesToArchiveBad(self):
        ...
        try:
            ...
            source = 'Training_Raw_Data_files_validated/Bad_Raw/'
                ...
                if (re.match(regex, filename)):
                    ...
                    if len(splitAtDot[3]) == LengthOfDateStampInFile:
                        if len(splitAtDot[4]) == LengthOfTimeStampInFile:
                            shutil.copy("Training_Batch_File/" + filename, "Training_Raw_Data_Files_Validated/Good_Raw")
                            self.logger.log(f, "Valid File name!! File moved to GoodData Folder :: %s" % filename)

                        else:
                            shutil.copy("Training_Batch_File/" + filename, "Training_Raw_Data_Files_Validated/Bad_Raw")
                    ...
                    shutil.copy("Training_Batch_Files/" + filename, "Training_Raw_Data_files_validated/Bad_Raw")
                    self.logger.log(f, "Invalid File name!!  File moved to Bad Data Folder :: %s" % filename)
            f.close()
    ...
    def validateColumnLength(self,NumberofColumns):
        ...
        try:
            ...
            for file in listdir('Training_Raw_Data_files_validated/Good_Raw/'):
                csv = pd.read_csv("Training_Raw_Data_files_validated/Good_Raw/" + file)
                if csv.shape[1] == NumberofColumns:
                    pass
                else:
                    shutil.move("Training_Raw_Data_files_validated/Good_Raw/" + file, "Training_Raw_Data_files_validated/Bad_Raw")
